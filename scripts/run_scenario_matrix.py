...
def prompt_version() -> str:
    version = settings.prompts_yaml.get("prompt_version")
    # The split-config branch no longer exports load_prompt_version and may
    # have a prompt file without the legacy top-level field. T0015.4 is the
    # frozen v1 baseline, so retain that explicit run stamp for this runner.
    return version.strip() if isinstance(version, str) and version.strip() else "v1"


def model_id() -> str:
    return settings.config_yaml["agent"]["react"]["model"]


def temperature() -> Any:
    return settings.config_yaml["agent"]["react"]["temperature"]


def configured_reasoning_effort() -> Any:
    return settings.config_yaml["agent"]["react"].get("reasoning_effort")


def configured_arm_efforts() -> dict[str, Any]:
    configured = settings.config_yaml.get("eval", {}).get("reasoning_ab", {})
    if not isinstance(configured, dict):
        raise ValueError("Missing 'eval.reasoning_ab' section in config/settings.yaml")
    return {name: configured.get(name) for name in ("baseline", "none")}


def validate_arm(arm: str) -> None:
    expected = configured_arm_efforts()[arm]
    actual = configured_reasoning_effort()
    if actual != expected:
        raise SystemExit(
            f"Configured agent.react.reasoning_effort={actual!r} does not match "
            f"--arm {arm!r} (expected {expected!r}). Update config/settings.yaml "
...
def report_header(run_date: str, version: str, arm: str | None = None) -> str:
    return "\n".join(
        [
            "# v1 Scenario Matrix",
            "",
            ...
            f"- reasoning_arm: `{arm or 'template'}`",
            f"- reasoning_effort: `{configured_reasoning_effort()}`",
...
def build_report(
    ...
    lines: list[str] = [report_header(run_date, version, arm)]
...
def run_template(path: Path = REPORT_PATH, arm: str | None = None) -> None:
    scenarios = load_scenarios()
    run_date = datetime.now(timezone.utc).replace(microsecond=0).isoformat()
    write_report(build_report(scenarios, observed=None, run_date=run_date, version=prompt_version(), arm=arm), path)
...
def run_live(
    ...
    observed = load_checkpoint(checkpoint_path)
    ...
    for index, scenario in enumerate(scenarios, start=1):
        ...
        save_checkpoint(observed, checkpoint_path)
        write_report(build_report(all_scenarios, observed, run_date_now(), version, arm), report_path)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="baseline",
        help="Reasoning-effort arm; validates agent.react.reasoning_effort before live calls.",
    )
    parser.add_argument(
        "--out",
        type=Path,
        default=None,
        help="Independent observed/checkpoint JSON path for this arm.",
    )
...
def main() -> None:
    args = parse_args()
    if args.template:
        run_template(arm=args.arm)
        return

    validate_arm(args.arm)
    out_path = args.out or ROOT / "evals" / f"reasoning_ab_{args.arm}.observed.json"
    ...
    try:
        ...
        raise SystemExit(
            ...
            f"{out_path}; re-run the same command to resume."
        ) from exc
    write_report(build_report(load_scenarios(), observed=observed, run_date=run_date_now(), version=version, arm=args.arm), report_path)
    print(f"Done. Report: {report_path}  Checkpoint: {out_path}")

...
**The T0020.4 coder slice is complete** — the cron-activation runbook is committed ([`docs/T0020.4_Cron_Activation_Runbook.md`](T0020.4_Cron_Activation_Runbook.md)) and the `## T0020` milestone + sub-ticket blocks are authored in `Tickets.md`. **Cron activation itself is maintainer-gated, not done here:** the nightly ingestion cron is **not** activated, and only fires once the maintainer clears the runbook's gates (D2 robots/ToS ratification, D6 Neon `alembic stamp head`, `workflow_dispatch` activation + first scheduled run, D10 live-vs-parked decision; D5's local Docker-Postgres portion is signed, the Neon portion remains). Follow the runbook, not this file, for the execution steps.

**The next coder work is the T0021 / T0022 track** (release integrity → honesty → v1.0 tag — see `research/v1-release-readiness-plan.md` §2 and the `[[v1-release-roadmap-m20-m22]]` memory note). Do not treat the T0021/T0022 track as begun until cron activation is signed off per the runbook §7.
...
- **T0020.4** — nightly ingestion cron activation. **The execution runbook is now committed** ([`docs/T0020.4_Cron_Activation_Runbook.md`](T0020.4_Cron_Activation_Runbook.md)) and enumerates every remaining maintainer gate: **D2** (robots/ToS ratification in a tracked doc), **D6** (one-time `alembic stamp head` on Neon's direct/non-pooled host), **activation** (`workflow_dispatch` from `main` → watch → confirm the first scheduled run), and **D10** (v1.0 live-vs-parked decision). **D5** (T0019.5 safety checks B–E) is signed for the local Docker-Postgres portion (2026-07-22, coder session) — the Neon/prod portion remains. GitHub only fires `schedule:` from the default branch and `.github/workflows/ingestion.yml` now lives on `main` (via PR #29), so activation is structurally possible; it is not automatic — the cron is **not** activated by the T0020.4 docs slice and remains dormant until the maintainer clears the gates.

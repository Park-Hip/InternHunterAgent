...
- T0008.3: Manual verification checklist (closes Milestone 8) — ran 12-question checklist against the live stack with seeded `clean_jobs`; recorded all observed answers in `docs/Manual_Verification_Guide.md`; confirmed 70 tests still pass (no regressions). Found 4 behavioral defects (DEF-1 tool invocation inconsistency, DEF-2 off-topic policy failure, DEF-3 greeting persona, DEF-4 two-turn refinement broken); all logged as follow-up tickets T0008.4/T0008.5/T0008.6. No source code changed.

## In progress
- None. T0008.3 closes Milestone 8. Follow-up prompt-fix tickets (T0008.4/T0008.5/T0008.6) need to be authored before implementation.
...
Author and implement T0008.4 (tool invocation reliability fix — covers DEF-1/DEF-4: role-filter/count/tech-filter/empty-result queries not triggering `query_clean_jobs`, and two-turn refinement broken). Then T0008.5 (off-topic policy enforcement — DEF-2 resume/off-topic requests fulfilled instead of declined). T0008.6 (greeting persona, low priority). After those fixes pass a re-run of the T0008.3 checklist, the MVP prompt-hardening work is complete and further work (larger dataset, resume/embedding retrieval, charts) requires new tickets against `Full_Design_Document.md` / `MVP_Spec.md` §6.

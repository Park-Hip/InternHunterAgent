# MVP Technical Design - Serving Path
...
> **Eviction:** A serving-path detail leaves when the implementation retires it or an owner document
...
- **Migrations deferred — *both deferral conditions have now fired* (2026-07-16).** The schema is
  ...
  which holds postings that have dropped out of search and cannot be re-fetched - meet the
...
## 7-8. Data Ingestion and Evaluation Harness - moved

Sections 7 and 8 describe the two **offline** pipelines, which the request path may never import
(`Full_Design_Document.md` §3). They live in
[`Offline_Pipelines_Design.md`](Offline_Pipelines_Design.md) and keep their original numbering, so
every `§7` and `§8` citation elsewhere in the repository still resolves. References to `§7` and
`§8` in the sections below point there.
...
the response status is already `200`** and cannot carry an error. Because the **`session`

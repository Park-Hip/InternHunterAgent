...
**Status (2026-07-19):** T0016.1–T0016.4, T0017.1–T0017.2, **T0018.1–T0018.4**, the SQL-generation reasoning-effort hotfix, and the ReAct/SQL-generation config split are complete on this stack; **T0019.1** is complete pending the maintainer's verdict confirmation; **T0019.2**, **T0019.3**, **T0019.4**, and **T0019.5** are complete; **T0019.7** (doc-only) is complete — the ping itself is not yet enabled, that's a maintainer action. **T0019.6** (nightly cron) remains the next code ticket; its workflow file exists in the working tree untracked but its documentation was lost to a concurrent-session collision (see the branch note above) and needs to be redone. Open issues live in [`Known_Issues.md`](Known_Issues.md); resolved/background items in [`Resolved_Issues.md`](Resolved_Issues.md).

**Milestone map (see `Tickets.md`):** T0013 freeze → T0016 security posture → T0017 streaming response delivery → T0018 clickable demo UI + go-live ✅ → **T0019 ingestion deploy readiness (live-DB) ◀ in progress (T0019.1 ✅, T0019.2 ✅, T0019.3 ✅, T0019.4 ✅, T0019.5 ✅, T0019.6 code-done/docs-lost, T0019.7 ✅ doc-only)**.

**Next recommended ticket:** **T0019.6** (redo its documentation — the workflow file survived) — see [Next recommended ticket](#next-recommended-ticket) below.
...
## Build/test status
Current-branch (T0019.7) results below; earlier per-ticket logs (T0011–T0018.2) are archived in [`archive/Repo_State_History.md`](archive/Repo_State_History.md).
...
**T0019.6 — Nightly ingestion cron.** T0019.5 landed the three preconditions the ticket brief called out: a pre-flight schema assertion that fails the run before any write on out-of-band drift, a `min_yield` floor that blocks a near-empty fetch from overwriting good rows, and a dead-man ping to healthchecks.io that fires only on the fully-green path. T0019 (Ingestion Deploy Readiness, live-DB) was scoped 2026-07-16 into eight sub-tickets: it replaces the static 50-row snapshot with a nightly refresh into the live Neon DB, and absorbs the three ops items previously deferred — the dead-man's switch (`deployment-research-plan.md` §9A, now implemented by T0019.5), the **keep-alive ping** decided 2026-07-16 (§1a, T0019.7's concern — a different ping, to keep Render warm, not to be confused with T0019.5's healthchecks.io dead-man ping), and the **schema-drift assertion/migration** re-pointed from T0018.4 (`Known_Issues.md` `[HIGH · PARTIALLY RESOLVED]` — write-path detection now closed by T0019.5; API-side read-path assertion remains open). Decision validation: [`research/ingestion-milestone-plan.md`](../research/ingestion-milestone-plan.md).

**T0019.1 is done** (2026-07-16, doc-only): recommended verdict **favorable, pending maintainer confirmation** — record in `research/deployment-research-plan.md` §11, evidence in `research/experiments/`. **T0019.2 is done** (2026-07-18): Alembic scaffolding + baseline migration, `Identity(always=True)` alignment in `models.py`, `scripts/reset_db.sql` demoted to local-dev-only. **T0019.3 is done** (2026-07-18): `clean_store.py`'s `TRUNCATE` is gone — the upsert (`upsert_clean_jobs`) is live and accumulate semantics hold; `is_active`/`first_seen_at`/`last_seen_at` are hidden lifecycle columns with a truthful backfill and a time-based expiry pass (`expire_stale_clean_jobs`) wired into the loader. **T0019.4 is done** (2026-07-18): per-page retry-then-skip resilience lives in `VietnamWorksSource`, and `run_ingestion` surfaces `pages_failed` in its summary — a single transient failure (429/5xx/timeout/transport error) no longer discards an entire run's already-fetched pages. **T0019.5 is done** (2026-07-19): `run_ingestion` now aborts before any write on schema drift or under-floor yield, and `main()` sends a dead-man ping to healthchecks.io only after a fully-green run; `pages_failed` stays unconsumed by design (logged as an open item in `Known_Issues.md`). **T0019.7 is done** (2026-07-19, doc-only, independent of .2–.6): the keep-alive-ping decision from §1a now has an executable runbook, a measurement template, and a pre-written decision rule in `Manual_Verification_Guide.md` — but the ping itself is not yet enabled and the idle-pool question stays open until the maintainer runs the 24 h observation. The safety rule below is now lifted.

**T0019.6 is code-complete but documentation-incomplete.** An earlier session wrote `.github/workflows/ingestion.yml` (nightly cron invoking the ingestion CLI) but its supporting docs were lost before being committed, in a collision with a concurrent session (see the T0019.7 completion report). The workflow file exists untracked in the working tree; a future session needs to review it, write its documentation (completion report, Manual Verification Guide entry, Known_Issues/deployment-research-plan updates), and commit it properly before it can be trusted as done.

**Maintainer action outstanding:** confirm T0019.1's favorable verdict before T0019.6 is built/finalized; separately, run the documented (not yet executed) `alembic stamp head` Neon-adoption command from `Manual_Verification_Guide.md` → T0019.2 § F, once, deliberately, before any future migration targets Neon; separately, execute the T0019.7 runbook (`Manual_Verification_Guide.md` → `### T0019.7`) to resolve the idle-pool question. None of these block each other.

**Sequencing:** .1 ✅ done (hard-blocks the cron in .6 only), .2 ✅ done, .3 ✅ done, .4 ✅ done, .5 ✅ done, **.6 code-done/docs-need-redo**, **.7 ✅ done** (doc-only, independent); .8 floats.

...
> **⚠ Reality check (2026-08-09).** Verified against GitHub and Render during an orientation audit:
>
> 1. **The nightly ingestion cron was never dormant.** It self-activated when PR #29 landed
...
**Reconciliation (2026-07-22, T0020.1):** `feature/t0019.10-job-details-allowlist` was merged to `main` as **PR #29**, and the local `main` ref was fast-forwarded to `origin/main`. **`origin/main` = `bcc81db`** is now the true head, carrying the full **T0019.6/.8/.9/.10** chain plus the **M13/M15 doc rescues**. The active working branch is now **`feature/t0020.1-main-reconciliation`** (this ticket's docs + local-ref follow-through). Render is unaffected — it still watches `feature/t0018.4-deploy` until the T0020.2 repoint.
...
- **M19** (T0019.1–.10) — Ingestion deploy readiness: robots/ToS gate, Alembic baseline + lifecycle columns, accumulate/upsert semantics, source resilience + retry, unattended-run safety, nightly cron (dormant/human-gated), keep-alive runbook, truthful `/ready` refresh date, ingestion coverage cap+interleave, `get_job_details` column allowlist. Landed on `main` via PR #29.
...
**T0020.2 — repoint the Render deploy source to `main` + track `render.yaml`.** With T0020.1 done (`main` reconciled to `bcc81db` via PR #29, local ref synced, docs corrected), the `main`-reconciliation gate (D1) is cleared — but Render still auto-deploys off `feature/t0018.4-deploy`, so the T0019 serving fixes (truthful `/ready` date, column allowlist, ingestion coverage) are not yet on the live path. T0020.2 moves Render's deploy branch to `main` and brings `render.yaml` under version control. **Do this before the cron can activate** — get the serving fixes onto the live instance ahead of any nightly run that could expire a row.

Then the rest of the T0020 sequence, all maintainer-gated:

- **T0020.3** — CI merge gate on `main` (`pre-deploy-refinement-plan.md` §6i); no automated gate today.
- **T0020.4** — nightly ingestion cron activation. GitHub only fires `schedule:` from the default branch; `.github/workflows/ingestion.yml` now lives on `main` (via PR #29) but its remaining human gates (D2 robots/ToS ratification, D5 live-DB unattended-safety checks) still bind before it is trusted to run unattended.

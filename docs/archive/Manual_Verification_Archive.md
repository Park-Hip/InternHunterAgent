# Manual Verification Archive - Completed Checklists

Reusable manual-verification checklists for completed tickets, moved from [`../Manual_Verification_Guide.md`](../Manual_Verification_Guide.md) during T0022.6.
This is distinct from [Manual_Verification_History.md](Manual_Verification_History.md), which contains dated live-run results rather than steps to repeat.
...
> The dated live-pass observed-results log (T0008.3) is archived in [`archive/Manual_Verification_History.md`](Manual_Verification_History.md).
...
> The dated live-pass observed-results log (T0009.8) is archived in [`archive/Manual_Verification_History.md`](Manual_Verification_History.md).
...
#### Part B - Recorded outcome (2026-08-10)

The maintainer confirmed that the job is enabled in ICT with the intended custom schedule
`*/12 7-22 * * *` and targets
`https://internhunteragent.onrender.com/api/v1/health`.
The checkpointer's idle pool does not hold Neon compute awake: Neon still suspends between the
health pings, so the Part C trigger did not fire.

| Metric | Recorded outcome |
|---|---|
| Neon idle-pool effect | Does not hold Neon compute awake; suspension gaps remain between pings. |
| Render warm window | 07:00 until about 23:03 ICT (last 22:48 ping plus 15-minute idle timer). |
| Render monthly projection | About 498 h/month, below the 750-hour Free cap. |
| First 07:00 request | Timed out after 13.37 s against cron-job.org's 30 s limit; this is the daily cold wake-up after overnight sleep. |
| A missed ping | Creates a 24-minute gap, which is long enough for Render to spin down. |
...
The trigger did **not** fire: compute stays low and suspension gaps are visible. No pool-setting
change is needed; the ping may remain running as configured.

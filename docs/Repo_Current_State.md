...
**Note on `.github/workflows/ingestion.yml` (T0019.6):** T0019.6 is **committed** on `feature/t0019.6-nightly-cron-finish` (`ad1c269`) — a separate branch not merged into this one, so the workflow file is absent from this working tree. That branch is dormant/unmerged pending `main` reconciliation and the D2/D5 maintainer gates; see `Tickets.md` → T0019.6. Not this ticket's scope to fix.
...
## Build/test status
Current-branch (T0019.10) results below; earlier per-ticket logs (T0011–T0018.3, incl. the ReAct/SQL-generation config split) are archived in [`archive/Repo_State_History.md`](archive/Repo_State_History.md).
...
**T0019.6 — GitHub Actions nightly ingestion cron.** It is the last open ticket in T0019 (.1–.5, .7, .8, .9, .10 are all complete), and it is **not primarily a coding task**: it is already committed on the separate `feature/t0019.6-nightly-cron-finish` branch (`ad1c269`), dormant/unmerged; what remains is landing that branch **plus three actions no coder session can perform**:

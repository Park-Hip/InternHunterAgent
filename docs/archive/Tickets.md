...
Ticket specs and delivery sequence for the MVP. The milestone index below keeps every milestone,
open or closed. Open tickets carry their **plan** here (Objective / In Scope / Out of Scope); with
M33 archived on 2026-08-19 none is open, and every plan now lives in
...
| 33 | T0033 | Vietnamese Language Milestone | ✅ ⚠ | Complete 2026-08-19, all five tickets merged the same day (.1 PR #77 the `Always answer in Vietnamese` rule, the vocabulary bridge and a `prompt_version` bump · .2 PR #78 the 19 glossary strings with their grader anchors, re-stating the v3 replay expectations in the same commit · .3 PR #79 the Vietnamese eval registry and row-aware purity grading · .5 PR #80 the tool literals and error strings · .4 PR #81 the demo UI, resolving its typography question by dropping Charter for a CSP-clean system serif stack). Shipped unmeasured: the `--arm A0 --runs 3` re-run never happened, and no 29-scenario Vietnamese capture has been graded - both carried in [`Repo_Current_State.md`](Repo_Current_State.md) without ticket numbers |
...
>
> ⚠ **M33:** every ticket shipped, but the milestone's own measurement did not. The D4 prerequisite
> - re-running the spike at `--arm A0 --runs 3` - was never done, so T0033.1's wording shipped
> without the control it was scoped against, and no 29-scenario Vietnamese capture has been graded.
> Neither carries a ticket number; both are recorded in
> [`Repo_Current_State.md`](Repo_Current_State.md) and need one before T0023's DoD sweep reads
> Vietnamese behavior as measured.

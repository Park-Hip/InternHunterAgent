...
## In progress / next
**This branch = Milestone 14 — Pre-Deploy Known-Issue Fixes** ([`Tickets.md`](Tickets.md) → T0014). Scope is **only** the deploy-facing items in [`Known_Issues.md`](Known_Issues.md) § "Config, startup & deployment" — deliberately **kept separate** from the broader §6 deploy-hardening body (that is **T0017 — Deploy Hardening**, deferred). Both sub-tickets are complete:

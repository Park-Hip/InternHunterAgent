...
**CI (T0020.3):** `.github/workflows/ci.yml` runs `uv run ruff check .` + `uv run mypy` + `uv run pytest -q` on every PR targeting `main` (the same three commands listed below). Enforcement as a required check is a pending maintainer branch-protection action — see `Known_Issues.md`.
...
**T0020.4 — nightly ingestion cron activation.** T0020.3 landed the CI merge gate (`.github/workflows/ci.yml`, ruff + mypy + pytest on PRs to `main`); the code deliverable is done, and only the maintainer branch-protection toggle remains to *enforce* it (logged in `Known_Issues.md`). Next in the T0020 sequence is activating the nightly ingestion cron: GitHub only fires `schedule:` from the default branch, and `.github/workflows/ingestion.yml` now lives on `main` (via PR #29), but its human gates (D2 robots/ToS ratification, D5 live-DB unattended-safety checks) still bind before it is trusted to run unattended.
...
- **T0020.2 maintainer actions (pending)** — Render dashboard: repoint the service deploy branch to `main` + redeploy; confirm `render.yaml`'s `name:` matches the existing service before any Blueprint sync (a mismatch mints a second Free service, eroding the 750 instance-hour margin); verify the live surface shows the T0019.10 fixes; confirm a no-op push to `main` triggers a redeploy.

...
Prior branch context below (carried from main at cut time):
...
The repo went from **55 local branches, 2 worktrees and 1 stash** to **2 branches, 1 worktree, no stash**. Only `feature/t0019.10-job-details-allowlist` (this branch) and `main` remain. Everything else was either merged into this branch, content-verified as superseded, or preserved by an archive tag.
...
## Build/test status
Latest results (T0021.1) at the top; the prior T0019.10 + cherry-picked T0019.6 results follow. Earlier per-ticket logs (T0011–T0018.3, incl. the ReAct/SQL-generation config split) are archived in [`archive/Repo_State_History.md`](archive/Repo_State_History.md).

- `uv run pytest -q` (T0021.1, full standard suite) → `335 passed, 8 skipped, 19 deselected, 4 subtests passed` in 525.66s. **+6 over the 329 baseline**, exactly accounting for the six net-new `tests/api/test_schema_guard.py` cases (correct schema, missing column, renamed column naming both directions, unexpected extra, empty result → table-missing, `OperationalError` → inspect failure). No pre-existing test was modified, weakened, or removed.
- `uv run pytest -q tests/api/test_schema_guard.py` (T0021.1, targeted) → `6 passed` in ~2.3s.
- `uv run ruff check .` (T0021.1, whole repo) → all checks passed.
- `uv run mypy` (T0021.1, whole repo) → the same 2 pre-existing, unrelated errors (`src/core/checkpointer.py:25`, `src/agents/runtime/middleware.py:48`, baselined by T0020.3). No third error introduced; `src/api/schema_guard.py` and `src/api/app.py` are clean.
- `git grep -n "services.ingestion" src/api/schema_guard.py` → no match; `git grep -n "services.ingestion" src/api/` → no match. Layer isolation (api → core only) confirmed: the serving guard pulls nothing from the ingestion package.
- **⚠ Manual verification (live-boot checks) NOT run this session.** The happy-boot and drift-fails-boot checks require Docker Postgres up; Docker was not available. The six automated cases patch `src.api.schema_guard.session_factory` and feed column sets, proving the guard's diff/exception logic, but do **not** prove the `information_schema` query returns the expected 22 columns against a real `clean_jobs`, nor that a live `ALTER TABLE ... RENAME COLUMN` aborts the boot end-to-end. Both checks are outstanding — recorded, not glossed. See `Manual_Verification_Guide.md` → T0021.1.

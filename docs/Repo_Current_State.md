## Current branch
`feature/t0019.4-source-resilience` — **T0019.4 Source resilience: per-page try/continue + retry/backoff.** Branched off `feature/t0019.3-accumulate-lifecycle`, which was branched off `feature/t0019.2-alembic-baseline`, which was branched off `feature/t0019.1-robots-tos-gate`, which was branched off `feature/t0018.4-deploy`, which remains the deployed branch — **LIVE: https://internhunteragent.onrender.com** (verified end-to-end 2026-07-16). Note `main` is stale at T0009; branch off `feature/t0018.4-deploy` (or its ticket-branch descendants), not `main`.
...
**Status (2026-07-18):** T0016.1–T0016.4, T0017.1–T0017.2, **T0018.1–T0018.4**, the SQL-generation reasoning-effort hotfix, and the ReAct/SQL-generation config split are complete on this stack; **T0019.1** is complete pending the maintainer's verdict confirmation; **T0019.2**, **T0019.3**, and **T0019.4** are complete. Open issues live in [`Known_Issues.md`](Known_Issues.md); resolved/background items in [`Resolved_Issues.md`](Resolved_Issues.md).

**Milestone map (see `Tickets.md`):** T0013 freeze → T0016 security posture → T0017 streaming response delivery → T0018 clickable demo UI + go-live ✅ → **T0019 ingestion deploy readiness (live-DB) ◀ in progress (T0019.1 ✅, T0019.2 ✅, T0019.3 ✅, T0019.4 ✅)**.

**Next recommended ticket:** **T0019.5** — see [Next recommended ticket](#next-recommended-ticket) below.
...
**Sequencing:** .1 ✅ done (hard-blocks the cron in .6 only), .2 ✅ done, .3 ✅ done, .4 ✅ done, then the spine **.5 → .6**; .7/.8 float.

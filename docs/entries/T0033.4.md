...
| `tests/api/test_static_serving.py` | 4 passed, 1 failed because its English assertion still expects `refresh date unavailable`. |
...
The static-serving test has one stale English assertion.

The test is red until its expectation is translated.

## Follow-ups

- Update the out-of-scope assertion in `tests/api/test_static_serving.py` when that path is owned.
- Run the successful streaming check after the other M33 tickets are merged.

## Known issues

- `KI-2026-08-19-static-test-english-assertion` **`[LOW · OPEN]` The static test asserts removed copy.**
  - **Found:** The focused suite failed only on the old `refresh date unavailable` assertion.
  - **Impact:** The focused test job is red even though the static UI and JavaScript are valid.
  - **Next:** Translate that assertion in the ticket that owns `tests/api/test_static_serving.py`.
  - **History:** None.

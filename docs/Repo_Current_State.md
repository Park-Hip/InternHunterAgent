...
**Status (2026-07-19):** T0016.1–T0016.4, T0017.1–T0017.2, **T0018.1–T0018.4**, the SQL-generation reasoning-effort hotfix, and the ReAct/SQL-generation config split are complete on this stack; **T0019.1** is complete pending the maintainer's verdict confirmation; **T0019.2**, **T0019.3**, **T0019.4**, and **T0019.5** are complete; **T0019.7** (doc-only) is complete — the ping itself is not yet enabled, that's a maintainer action. **T0019.8** is complete (code + docs; live-DB manual checks outstanding — see Build/test status). **T0019.9** is complete in code, config and tests, but its `[MED]` `Known_Issues` entry stays open as **PARTIALLY RESOLVED**: confirming the new `max_jobs: 150` actually clears the API's current ceiling needs a live re-measure, which decision **D8** blocks — runbook with empty result slots at `research/data-ingestion-stage.md` §11. **T0019.10** was scoped 2026-07-20 (decision D3) to give a real ticket ID to a reference the design doc already carried. **T0019.6** (nightly cron) is **not** simply "next": its workflow file exists untracked, its documentation was lost to a concurrent-session collision, and it is blocked on two maintainer actions (D2 — ratify the T0019.1 ToS verdict in a tracked doc; D5 — run T0019.5's manual checks B–E against a live Postgres). T0019.10 should also land before the cron is enabled. Open issues live in [`Known_Issues.md`](Known_Issues.md); resolved/background items in [`Resolved_Issues.md`](Resolved_Issues.md).

**Milestone map (see `Tickets.md`):** T0013 freeze → T0016 security posture → T0017 streaming response delivery → T0018 clickable demo UI + go-live ✅ → **T0019 ingestion deploy readiness (live-DB) ◀ in progress (T0019.1 ✅, T0019.2 ✅, T0019.3 ✅, T0019.4 ✅, T0019.5 ✅, T0019.6 workflow-written/docs-lost/human-gated, T0019.7 ✅ doc-only, T0019.8 ✅, T0019.9 ✅ code-complete/re-measure gated on D8, T0019.10 scoped 2026-07-20)**.

**Next recommended ticket:** **T0019.10**, then T0019.6 once its human gates clear — see [Next recommended ticket](#next-recommended-ticket) below.
...
## Build/test status
Current-branch (T0019.9) results below; earlier per-ticket logs (T0011–T0018.3, incl. the ReAct/SQL-generation config split) are archived in [`archive/Repo_State_History.md`](archive/Repo_State_History.md).
...
**T0019.10 — `get_job_details` explicit column allowlist.** Small, self-contained, no external dependency, and **must land before the nightly cron is enabled**: `fetch_job_details` (`src/services/query/job_details.py:16`) runs `SELECT * FROM clean_jobs` and hands the agent every column, including the `is_active`/`first_seen_at`/`last_seen_at` lifecycle columns that T0019.3 deliberately kept out of `schema_context`. Cosmetic today only because every row is `is_active = true`; it becomes a real honesty defect the moment expiry starts flipping rows, since the agent could describe stale-listing state through an unguarded path while the sanctioned path still hides it. Scope in [`Tickets.md`](Tickets.md) → T0019.10.

**Why not T0019.6 (the previous recommendation):** it is not a coding task. Its workflow file already exists (untracked); what it needs is its lost documentation rewritten **plus two maintainer actions that no coder session can perform**:
- **D2** — ratify T0019.1's favorable robots/ToS verdict in a *tracked* document. The T0019.1 report says "pending maintainer confirmation," and no committed doc records a confirmation.
...
Note the cron also cannot fire at all until the branch chain reaches `main` — GitHub only triggers `schedule:` from the default branch — so `main` reconciliation (D1) is sequenced work ahead of it, not background hygiene.

**Also outstanding from T0019.9:** the coverage fixes have landed, but the `[MED · PARTIALLY RESOLVED]` entry does not close until the re-measure in [`research/data-ingestion-stage.md`](../research/data-ingestion-stage.md) §11 is run and §11.2's table filled from a real run. That needs decision **D8** settled first. If the measured yield comes back at or above ~150, the cap is binding again and the original defect has returned. Two smaller follow-ups were newly logged in [`Known_Issues.md`](Known_Issues.md): budget still spent re-confirming known postings (needs a layer decision — a DB read inside the source adapter), and a truncated run surfacing no signal.

**Also outstanding from T0019.8:** its manual checks A–E were not run (Docker Desktop was down this session). Check B — endpoint date vs. `psql` `MAX(last_seen_at)` — is the load-bearing one. See [`Manual_Verification_Guide.md`](Manual_Verification_Guide.md) → T0019.8.

**T0019.9** (ingestion coverage: raise `max_jobs` + round-robin interleave) is scoped but gated on the **D8** ToS-posture decision, since it deliberately raises request volume against the same host. Full gap analysis and the D1–D13 decision list: [`research/v1-release-readiness-plan.md`](../research/v1-release-readiness-plan.md).
